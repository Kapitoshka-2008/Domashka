...
def filter_by_state(data, state='EXECUTED'):
    """ Фильтрует список словарей по заданному состоянию. :param data: Список словарей :param state: Значение состояния, по которому будет производиться фильтрация (по умолчанию 'EXECUTED') :return: Новый список словарей, соответствующих указанному состоянию """
    return [item for item in data if item.get('state') == state]


def sort_by_date(data, order='descending'):
    """ Сортирует список словарей по дате. :param data: Список словарей :param order: Порядок сортировки ('ascending' или 'descending'; по умолчанию 'descending') :return: Отсортированный список словарей """
